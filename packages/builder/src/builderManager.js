...
export class BuilderManager {
    ...
    getPlugin(name) {
        ...
        if (!plugin.builder) {
            logger.error(`Registered plugin '${name}' does not provide a builder implementation.`);
            throw new Error(`Registered plugin '${name}' does not provide a builder implementation.`);
        }
    ...
    getDefaultPlugin() {
        ...
        try {
            ...
            logger.error(`Failed to get default builder plugin '${defaultPluginName}':`, error);
            throw new Error(`Default builder plugin '${defaultPluginName}' could not be loaded.`, { cause: error });
        }
    }
    /**
