...
import { Recipe, getRecipe } from '@devx/recipes';
...
export default class Init extends BaseCommand {
  ...
  async run(): Promise<void> {
    // Parse recognized flags. We handle positional args manually via this.argv below.
    const { flags } = await this.parse(Init);

    const forceFlag = flags.force;
    const nameFlag = flags.name;
    const recipeFlag = flags.recipe;

    // Since strict = false, this.argv contains all args including flags and key=value pairs
    const rawKeyValues = this.argv.filter(
      (arg) => arg.includes('=') && !arg.startsWith('-')
    );
    const keyValueArgs = this.parseKeyValueArgs(rawKeyValues);

    const targetDir = process.cwd(); // Initialize in current directory
    const targetPath = path.join(targetDir, '.stack.yml');
    ...
    if (existsSync(targetPath)) {
      if (!forceFlag) {
        const { confirm } = await prompts({
          type: 'confirm',
          name: 'confirm',
          message: `Configuration file already exists at ${targetPath}. Overwrite?`,
          initial: false,
        });
        if (!confirm) {
      ...
      this.log(`Overwriting existing file: ${targetPath}`);
    }
   ...
   */
  private async runRecipeInteractiveInit(
    recipe: Recipe
  ): Promise<Record<string, string>> {
    const questions: PromptObject[] = Object.entries(recipe.options || {}).map(
      ([key, option]) => ({
