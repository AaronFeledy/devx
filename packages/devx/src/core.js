...
import { getStackState, updateStackState, removeStackState, getInitialStackState, StackBuildStatus, } from './state/index.js';
...
async function loadStack(stackIdentifier) {
    try {
        ...
        if (!result) {
            throw new Error(`Stack configuration not found for '${stackIdentifier}'.`);
        }
        ...
        logger.error(`Failed to load stack configuration for '${stackIdentifier}': ${errorMessage}`, error);
        throw new Error(`Failed to load stack configuration for '${stackIdentifier}'.`, { cause: error });
    }
}
/**
...
async function getBuilder(stackConfig) {
    ...
    if (!builderName) {
        throw new Error(`No builder specified for stack '${stackConfig.name}' and no default builder configured.`);
    }
    try {
        ...
        logger.error(`Failed to get builder plugin '${builderName}' for stack '${stackConfig.name}'`, error);
        throw new Error(`Failed to get builder plugin '${builderName}' for stack '${stackConfig.name}'`, { cause: error });
    }
}
/**
...
async function getEngine(stackConfig) {
    ...
    if (!engineName) {
        throw new Error(`No engine specified for stack '${stackConfig.name}' and no default engine configured.`);
    }
    try {
        return engineManager.getPlugin(engineName);
    }
    catch (error) {
        logger.error(`Failed to get engine plugin '${engineName}' for stack '${stackConfig.name}'`, error);
        throw new Error(`Failed to get engine plugin '${engineName}' for stack '${stackConfig.name}'`, { cause: error });
    }
}
/**
...
export async function build(stackIdentifier) {
    ...
    logger.info(`Building stack '${stackConfig.name}' using builder '${builder.name}'...`);
    try {
...
export async function start(stackIdentifier) {
    ...
    if (!state || state.buildStatus !== StackBuildStatus.Built) {
        logger.warn(`Stack '${stackConfig.name}' is not built or build status is unknown. Attempting to build first...`);
        try {
            ...
            logger.error(`Build failed for stack '${stackConfig.name}', cannot start.`, buildError);
            throw new Error(`Build failed for stack '${stackConfig.name}', cannot start.`, { cause: buildError });
        }
    }
    logger.info(`Starting stack '${stackConfig.name}' using builder '${builder.name}'...`);
    try {
...
export async function stop(stackIdentifier) {
    ...
    const builder = await getBuilder(stackConfig);
    const projectPath = dirname(configPath);
    logger.info(`Stopping stack '${stackConfig.name}' using builder '${builder.name}'...`);
    try {
...
export async function destroy(stackIdentifier, options) {
    ...
    const builder = await getBuilder(stackConfig);
    const projectPath = dirname(configPath);
    logger.info(`Destroying stack '${stackConfig.name}' using builder '${builder.name}'...`);
    try {
        await builder.destroy(stackConfig, projectPath, options);
        logger.info(`Stack '${stackConfig.name}' resources destroyed successfully.`);
...
export async function status(stackIdentifier) {
    ...
    const projectPath = dirname(configPath);
    logger.debug(`Getting status for stack '${stackConfig.name}' using engine '${engine.name}'...`);
    try {
        const statusInfo = await engine.getStackStatus(stackConfig.name, projectPath);
        await updateStackState(stackConfig.name, {
            runtimeStatus: statusInfo.status,
            lastError: statusInfo.status === StackStatus.Error ? statusInfo.message : null,
        });
