// TODO: Fix bun:test mocking issues with preload and overrides.
// The tests in this file are currently failing due to difficulties in reliably
// mocking native modules (fs, fetch) and local module functions (execAsync)
// using bun test --preload.

import { describe, expect, test, mock, afterAll, beforeEach } from 'bun:test';
import { PodmanPlugin } from '../src/plugins/podman/index'; // Adjusted import path
import { join } from 'path';
import { homedir } from 'os';
import type { PlatformInfo } from '../src/plugins/podman/platform'; // Adjusted import path

// --- Mocking external dependencies ---

// Mock platform module
const mockPlatformExecAsync = mock(async () => ({ stdout: '', stderr: '' }));
const mockGetPlatformInfo = mock(
  async (): Promise<PlatformInfo> => ({
...
const mockGetPlatformSpecificConfig = mock(
  () => '[engine]\nevents_logger = "file"\ncgroup_manager = "cgroupfs"'
); // Provide default engine config
mock.module('../src/plugins/podman/platform', () => ({
  // Adjusted mock path
  ...require('../src/plugins/podman/platform'),
...
describe('PodmanPlugin', () => {
  ...
  beforeEach(async () => {
    ...
    mockInjectedExecAsync.mockImplementation(
      async (_command: string, _options?: any) => {
        return { stdout: 'Success', stderr: '' };
      }
    );
  ...
  test('initialize creates necessary directories and config files', async () => {
    ...
    expect(mockWriteFile).toHaveBeenCalledWith(
      expect.stringContaining('storage.conf'),
      expect.any(String)
    );
    expect(mockWriteFile).toHaveBeenCalledWith(
      expect.stringContaining('containers.conf'),
      expect.any(String)
    );
  ...
  test('initialize configures Podman storage.conf correctly', async () => {
    ...
    const writeCall = mockWriteFile.mock.calls.find(
      (call) => typeof call[0] === 'string' && call[0].endsWith('storage.conf')
    );
    expect(writeCall).toBeDefined();

    const writtenConfig = writeCall![1] as string;
    expect(writtenConfig).toContain(
      `graphroot = "${join(testRoot, 'podman')}"`
    );
  ...
  test('initialize configures Podman containers.conf correctly', async () => {
    ...
    const writtenConfig = writeCall![1] as string;
    expect(writtenConfig).toContain(
      `volumes = ["${join(testRoot, 'podman', 'volumes')}:${join(testRoot, 'podman', 'volumes')}"]`
    );
  ...
  test('start calls injected execAsync with correct command', async () => {
    ...
    expect(execCall).toContain(
      join(testRoot, 'bin', 'podman', 'bin', 'podman')
    ); // Check correct binary path
  ...
  test('status calls injected execAsync with correct command', async () => {
    ...
    expect(execCall).toContain(
      `ps --filter name=test-container --format {{.Status}}`
    );
    expect(execCall).not.toContain(`--root`);
    expect(execCall).not.toContain(`--runroot`);
  });
