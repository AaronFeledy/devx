...
export class EngineManager {
    ...
    getPlugin(name) {
        ...
        if (!plugin.engine) {
            logger.error(`Registered plugin '${name}' does not provide an engine implementation.`);
            throw new Error(`Registered plugin '${name}' does not provide an engine implementation.`);
        }
    ...
    getDefaultPlugin() {
        ...
        try {
            ...
            logger.error(`Failed to get default engine plugin '${defaultPluginName}':`, error);
            throw new Error(`Default engine plugin '${defaultPluginName}' could not be loaded.`, { cause: error });
        }
    }
    /**
