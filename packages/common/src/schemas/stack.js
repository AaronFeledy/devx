...
const PortMappingSchema = z.union([
    z
        .string()
        .regex(/^\d+(-\d+)?:\d+(-\d+)?(\/(tcp|udp))?$/, 'Invalid port mapping format (e.g., "8080:80", "8080-8081:80-81", "53:53/udp")'), // "8080:80", "8080-8081:80-81", "53:53/udp"
...
const VolumeMappingSchema = z
    .string()
    .regex(/^.+:.+(:ro|:rw)?$/, 'Invalid volume mapping format (e.g., "./local:/app", "my_volume:/data:ro")'); // "local/path:container/path", "volume_name:/data:ro"
/** Represents an environment variable definition in KEY=value format. */
const EnvironmentVariableSchema = z
    .string()
    .regex(/^[^=]+=.*$/, 'Invalid environment variable format (e.g., "NODE_ENV=development")'); // "KEY=value"
